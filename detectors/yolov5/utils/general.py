...
from detectors.yolov5.utils import TryExcept, emojis
from detectors.yolov5.utils.downloads import curl_download, gsutil_getsize
from detectors.yolov5.utils.metrics import box_iou, fitness
...
def check_amp(model):
    ...
    from detectors.yolov5.models.common import AutoShape, DetectMultiBackend
...
def non_max_suppression(
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # Box/Mask
        box = xywh2xyxy(x[:, :4].clone())  # center_x, center_y, width, height) to (x1, y1, x2, y2)
        mask = x[:, mi:].clone()  # zero columns if no masks
        ...
        if not n:  # no boxes
            continue
        x = x[x[:, 4].argsort(descending=True)[:max_nms]].clone()  # sort by confidence and remove excess boxes

        # Batched NMS
        c = x[:, 5:6].clone() * (0 if agnostic else max_wh)  # classes
        boxes, scores = x[:, :4].clone() + c, x[:, 4].clone()  # boxes (offset by class), scores
        ...
        if merge and (1 < n < 3e3):  # Merge NMS (boxes merged using weighted mean)
            ...
            x[i, :4] = torch.mm(weights, x[:, :4].clone()).float() / weights.sum(1, keepdim=True)  # merged boxes
            if redundant:
