...
from detectors.yolov5.utils.downloads import attempt_download
...
def attempt_load(weights, phase, device=None, inplace=True, fuse=True):
    """
    ...
    # Joint training for better transferability to architectures with transformer
    if isinstance(weights, str) and any(size in weights for size in ['yolov5']):
        model_sizes = ['m'] if phase == "phase1" else ['n', 's', 'm', 'l', 'x']
        weights = [f"{weights}/yolov5{size}.pt" for size in model_sizes]
