...
def attack_one_step(detector, augmentor, renderer, generator, attack_class, render_batch_size, detect_batch_size,
                    angles, npz_list, label, device, spe_texture, dataset_dir):
    # obtain perturbation [perturbation_size, perturbation_size, 4]
    perturbation = generator.subgen_sta_texture(angles)
    # obtain adv_texture for phase1 [texture_size, texture_size, 3]
    adv_texture = generator.gen_phase1_adv_texture(spe_texture, perturbation)
    ...
    closs = preds[:, 4].sum()
    loss = closs + (1e-4 if closs < 1e-3 else 0)   # avoid zero

    return loss, imgs, preds, labels, perturbation, adv_texture


def SDCA_attack_phase1(config):
    ...
    # set log
    writer, logger = set_log(new_subfolder, "phase1")
    ...
    # load dataset
    npz_list = list(map(int, np.load(config['dataset']['stg1_idx'])['data']))
    # load labels
    ...
    # generate random angles for obtaining gradient sets
    angles = [torch.tensor(2 * np.pi * np.random.rand(freq + 1, freq + 1),
                           dtype=torch.float32, device=device, requires_grad=True) for freq in config['phase1_train']['freqs']]

    # set optimizer for angles with different scales
    optimizer = torch.optim.Adam([
        {'params': angles[0], 'lr': config['phase1_train']['lr'][0]},
        {'params': angles[1], 'lr': config['phase1_train']['lr'][1]},
        {'params': angles[2], 'lr': config['phase1_train']['lr'][2]},
    ], weight_decay=config['phase1_train']['weight_decay'])

    # initialize detector
    detector = attempt_load(config['general_train']['detector_dir'], "phase1", device).eval()
    ...
    # initialize SDG
    generator = SDG(device, config, "phase1")
    # generate speckle texture
    spe_texture = generator.subgen_spe_texture(new_subfolder)
    ...
    logger.info("Start training phase1...")
    ...
    iterations = config['phase1_train']['epoch']
    for epoch in range(iterations):
        optimizer.zero_grad()

        loss, imgs, preds, labels, perturbation, adv_texture = attack_one_step(detector, augmentor, renderer, generator,
                                                                               attack_class, render_batch_size,
                                                                               detect_batch_size, angles, npz_list,
                                                                               label, device, spe_texture, dataset_dir)

        writer.add_scalar('Training/Loss_cls', loss.item(), epoch)
        ...
        with torch.no_grad():
            ...
            if epoch % 20 == 0 and config['save']['save_intermediate_results']:
                ...
                save_texture(epoch, os.path.join(new_subfolder, 'texture'), adv_texture, perturbation)

            if epoch % 20 == 0:
                adv_texture_tb = adv_texture.clone().detach().permute(2, 0, 1)
                perturbation_tb = perturbation.clone().detach().permute(2, 0, 1)
                writer.add_image('Texture/adv_texture', adv_texture_tb, epoch)
                writer.add_image('Texture/starry_texture_Sp', perturbation_tb, epoch)

            # record time
            end_time = time.perf_counter()
            logger.info(f"[Epoch {epoch + 1}] loss_cls: {loss:.4f}, time: {end_time - pre_time:.2f}s")
            pre_time = end_time
    ...
    logger.info(f"Best: Epoch {best_epoch}, loss: {best_loss:.4f}")
    draw_detection_results(new_subfolder, best_imgs, best_preds, detect_batch_size, best_labels)
    save_texture(best_epoch, os.path.join(new_subfolder, 'best'), best_adv_texture, best_perturbation)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run SDCA phase 1 training.')
